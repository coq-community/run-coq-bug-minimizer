...
comment_contents+="$(print_file "$(( 2 * ${GITHUB_MAX_CHAR_COUNT} / 5 ))" "build log" "${start_code}" "$3" "${end_code}")"
comment_contents+="$(print_file "$(( 2 * ${GITHUB_MAX_CHAR_COUNT} / 5 ))" "minimizer log" "${start_code}" "$4" "${end_code}")"
