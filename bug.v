...
Proof.
repeat intro; subst; apply update_nth_ext; trivial.
Qed.

Global Instance update_nth_Proper_eq {A} : Proper (eq ==> (eq ==> eq) ==> eq ==> eq) (@update_nth A) | 1.
Proof.
repeat intro; subst; apply update_nth_Proper; repeat intro; eauto.
Qed.
...
Lemma update_nth_id_eq : forall {T} f (H : forall x, f x = x) n (xs : list T),
    update_nth n f xs = xs.
Proof.
intros; apply update_nth_id_eq_specific; trivial.
Qed.
...
Proof.
intros; apply update_nth_id_eq; trivial.
Qed.
...
Proof.
intros; apply length_update_nth.
Qed.
...
Definition IMPOSSIBLE {T} : list T.
exact nil.
Qed.
...
Proof.
intros T n x xs; rewrite splice_nth_equiv_update_nth with (f := fun _ => x); auto.
Qed.
...
Proof.
intros T n x xs H; rewrite splice_nth_equiv_update_nth_update with (f := fun _ => x); auto.
Qed.
...
Proof.
intros T n x xs H; rewrite splice_nth_equiv_update_nth_snoc with (f := fun _ => x); auto.
Qed.
...
Proof.
rewrite firstn_skipn_add; do 2 f_equal; auto with arith.
Qed.
#[global]
...
Proof.
induction len as [|len IHlen]; intros; simpl; rewrite ?IHlen; reflexivity.
Qed.
...
Proof.
  ...
  - simpl; rewrite H by lia; replace (n + (m - n))%nat with m by lia.
    ...
    intros; rewrite Nat.sub_succ; apply H; lia.
Qed.

Lemma map_seq_pred n m :
  seq n m = map (fun i => (i - 1)%nat) (seq (S n) m).
Proof.
rewrite <- map_id at 1; apply map_seq_ext; intros; lia.
Qed.

Lemma map_seq_succ n m :
  seq (S n) m = map (fun i => (i + 1)%nat) (seq n m).
Proof.
rewrite <- map_id at 1; symmetry; apply map_seq_ext; intros; lia.
Qed.
...
Proof.
intros; rewrite update_nth_nth_default_full; repeat break_match; boring.
Qed.
...
Proof.
intros; apply update_nth_nth_default_full; assumption.
Qed.
...
Proof.
intros; apply update_nth_nth_default; assumption.
Qed.
...
Proof.
induction x; rewrite ?app_nil_l, <-?app_comm_cons; autorewrite with push_sum; lia.
Qed.
#[global]
...
Proof.
induction x; cbn [rev]; autorewrite with push_sum; lia.
Qed.
#[global]
...
Proof.
cbv [respectful]; repeat intro; subst; apply fold_right_Proper; repeat intro; eauto.
Qed.
...
Proof.
cbv [respectful]; repeat intro; subst; apply fold_left_Proper; repeat intro; eauto.
Qed.
Import Coq.Lists.SetoidList.
...
Proof.
intros; apply Forall2_forall_iff; assumption.
Qed.
...
Proof.
induction xs; cbn in *; [ reflexivity | rewrite IHxs; f_equal ]; intros; intuition auto.
Qed.
Global Instance flat_map_Proper A B : Proper (pointwise_relation _ eq ==> eq ==> eq) (@flat_map A B).
Proof.
repeat intro; subst; apply flat_map_ext; auto.
Qed.

Global Instance map_Proper_eq {A B} : Proper ((eq ==> eq) ==> eq ==> eq) (@List.map A B) | 1.
Proof.
repeat intro; subst; apply pointwise_map; repeat intro; eauto.
Qed.
Global Instance flat_map_Proper_eq {A B} : Proper ((eq ==> eq) ==> eq ==> eq) (@List.flat_map A B) | 1.
Proof.
repeat intro; subst; apply flat_map_Proper; repeat intro; eauto.
Qed.
...
Proof.
repeat intro; subst; apply partition_Proper; repeat intro; eauto.
Qed.
...
Proof.
induction ls as [|x xs IHxs]; cbn; [ | rewrite IHxs ]; reflexivity.
Qed.
Lemma flat_map_app A B (f : A -> list B) xs ys
  : flat_map f (xs ++ ys) = flat_map f xs ++ flat_map f ys.
Proof.
induction xs as [|x xs IHxs]; cbn; rewrite ?IHxs, <- ?app_assoc; reflexivity.
Qed.
#[global]
...
Proof.
induction xs as [|x xs IHxs]; cbn; rewrite ?map_app; congruence.
Qed.
...
Proof.
rewrite flat_map_rev; setoid_rewrite rev_involutive; reflexivity.
Qed.
#[global]
...
Proof.
induction xs as [|x xs IHxs]; cbn; [ | intro H; rewrite IHxs, H by eauto ]; case b'; reflexivity.
Qed.
...
Proof.
revert v; induction xs; cbn; intros; congruence.
Qed.
...
Proof.
revert ls2; induction ls1, ls2; cbn; f_equal; eauto.
Qed.
...
Proof.
revert f v; induction ls; cbn; eauto.
Qed.
...
Proof using Type.
induction n; cbn; f_equal; assumption.
Qed.
...
Lemma eq_firstn_nat_rect {A} n ls
  : @List.firstn A n ls
    = nat_rect
        ...
        n ls.
Proof using Type.
revert ls; induction n, ls; cbn; f_equal; auto.
Qed.
...
Lemma eq_combine_list_rect {A B} xs ys
  : @List.combine A B xs ys
    = list_rect
        ...
        xs ys.
Proof using Type.
revert ys; induction xs, ys; cbn; f_equal; auto.
Qed.
...
Lemma eq_update_nth_nat_rect {A} n f xs
  : @update_nth A n f xs
    = (nat_rect _)
        ...
        xs.
Proof using Type.
revert xs; induction n, xs; cbn; f_equal; auto.
Qed.
...
Proof.
induction ls as [|?? IH]; cbn; now f_equal.
Qed.
...
Proof.
rewrite <- !fold_left_rev_right; now rewrite_fold_right_fun_apply.
Qed.
...
Proof.
cbv [span_cps]; rewrite span_cps'_id; destruct span; reflexivity.
Qed.
...
Proof.
cbv [span span_cps]; cbn; rewrite !span_cps'_id; cbv [id]; reflexivity.
Qed.

Lemma span_app {A} f xs
  : fst (@span A f xs) ++ snd (@span A f xs) = xs.
Proof.
induction xs; rewrite ?span_nil, ?span_cons; break_innermost_match; boring.
Qed.
...
Proof.
induction xs; cbn; break_innermost_match; boring; inversion_list; inversion_one_head Forall; congruence.
Qed.
...
Proof.
symmetry; apply fold_right_higher_order.
Qed.
...
Proof.
symmetry; apply fold_left_higher_order.
Qed.
...
Qed.
    ...
    Proof.
      intros x l.
rewrite reifiable_existsb_is_existsb.
...
reflexivity.
    ...
    Proof.
      intros.
rewrite <- existsb_eqb_true_iff.
split.
      - intros H.
rewrite H.
auto.
      - intros H.
destruct (existsb (eqb x) l).
        + exfalso.
apply H.
reflexivity.
        + reflexivity.
    ...
    Proof.
      ...
      - reflexivity.
      - simpl.
destruct (existsb (eqb x') (nodupb l')) eqn:E.
        + rewrite existsb_eqb_true_iff in E.
split.
          -- intros [H|H].
            ++ rewrite <- H.
apply E.
...
rewrite IHl'.
apply H.
        + rewrite existsb_eqb_false_iff in E.
split.
          -- intros [H|H].
            ++ rewrite H.
simpl.
...
intros [H|H].
                +++ rewrite H in *.
rewrite H1 in E.
apply existsb_eqb_false_iff in E.
apply E.
...
End Crypto_DOT_Util_DOT_ListUtil_WRAPPED.
Module Export Crypto_DOT_Util_DOT_ListUtil.
Module Export Crypto.
Module Export Util.
Module ListUtil.
Include Crypto_DOT_Util_DOT_ListUtil_WRAPPED.ListUtil.
End ListUtil.
...
End Crypto_DOT_Util_DOT_ListUtil.
