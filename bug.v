...
(* coqc version 8.20+alpha compiled with OCaml 4.14.1
   ...
   Expected coqc runtime on this file: 33.413 sec *)
...
Module mathcomp_DOT_classical_DOT_functions_WRAPPED.
Module functions.
 
Import mathcomp.ssreflect.all_ssreflect mathcomp.finmap.finmap mathcomp.algebra.ssralg mathcomp.algebra.ssrnum mathcomp.algebra.ssrint mathcomp.algebra.rat.
Import HB.structures.
Import mathcomp.classical.mathcomp_extra mathcomp.classical.boolp mathcomp.classical.classical_sets.
...
Lemma invK {aT rT} {A : set aT} {B : set rT} {f : {splitsurj A >-> B}} :
   {in B, cancel f^-1 f}.
Proof.
by move=> x Bx; rewrite -[x in RHS]'oinvK_f// -some_inv/=.
Qed.
...
Lemma invS {aT rT} {A : set aT} {B : set rT} {f : {splitsurj A >-> B}} :
  {homo f^-1 : x / B x >-> A x}.
Proof.
by move=> x /'oinvS_f/= [a Aa]; rewrite -some_inv => -[<-].
Qed.
...
Lemma funK {aT rT : Type} {A : set aT} {s : {splitinj A >-> rT}} :
  {in A, cancel s s^-1}.
Proof.
by move=> x Ax; apply: Some_inj; rewrite some_inv funoK.
Qed.
...
HB.builders Context {aT rT} A (f : aT -> rT) of @Inv_Can _ _ A f.
  ...
  Proof.
by rewrite -oliftV/=; apply: can_in_pcan; apply: funK.
Qed.
...
HB.builders Context {aT rT} {A : set aT} {B : set rT} (f : aT -> rT)
  ...
  Proof.
by move=> x Bx; rewrite -some_inv/= invK.
Qed.
  #[local] Lemma oinvS : {homo 'oinv_f : x / B x >-> (some @` A) x}.
  Proof.
by move=> x /invS Af'x; exists (f^-1 x); rewrite // -some_inv.
Qed.
...
Proof.
by split=> [x|x /set_mem] [a Aa <-]//=; exists a.
Qed.
...
Proof.
by rewrite inv_oapp -oliftV.
Qed.

Lemma oapp_can_subproof (f : {inj A >-> rT}) : Inv_Can _ _ (some @` A) (oapp f).
Proof.
by split=> x /set_mem[a Aa <-]/=; rewrite inv_oapp funoK ?inE.
Qed.
...
Proof.
by rewrite funeqE => x; rewrite oinv_comp -!oliftV.
Qed.
...
Proof.
by split=> x Ax; rewrite oinv_comp/= funoK ?mem_fun//= funoK.
Qed.
...
HB.builders Context {aT rT} {A : set aT} {B : set rT} (f : aT -> rT) of SplitInjFun_CanV _ _ A B f.
  ...
  Proof.
by move=> x Bx; apply: injV; rewrite ?funK ?(mem_fun, mem_inv).
Qed.
  HB.instance Definition _ := Inv_CanV.Build aT rT A B f invS invK.
HB.end.
...
HB.builders Context {aT rT} f of BijTT aT rT f.
  ...
  Proof.
by apply: cid; case: bij => g; exists g.
Qed.
...
Proof.
by move=> b /'oinvP_f[x Ax _]; exists x.
Qed.
...
Proof.
by split=> x Ax; rewrite splitV funoK.
Qed.
...
Proof.
by apply/funext => x; apply: Some_inj; rewrite splitV -oliftV.
Qed.

Lemma splitsurj_subproof (f : {surj A >-> B}) : Inv_CanV _ _ A B (split f).
Proof.
by split=> [+|+ /set_mem] => b Bb; rewrite splitV; case: oinvP.
Qed.
...
HB.builders Context {aT rT} {A : set aT} {B : set rT} (f : aT -> rT) of
  ...
  Proof.
 by move=> x Ax; apply: inj; rewrite ?invK ?mem_fun.
Qed.
...
Proof.
by exists [injfun of g]; apply/funP.
Qed.
End funPinj.
...
Proof.
by exists [surjfun of g]; apply/funP.
Qed.
End funPsurj.
...
Proof.
by exists [splitinjfun of f']; apply/funP.
Qed.
End funPsplitinj.
...
Proof.
by rewrite /unbind; split=> x /'funS_f [y Bu <-].
Qed.
...
Proof.
by split=> x _; rewrite oinv_sigR/= set_valE/= funoK ?inE.
Qed.
...
Proof.
by split=> x _; rewrite /er oinv_valR/= funoK/= ?funoK ?inE.
Qed.
...
End mathcomp_DOT_classical_DOT_functions_WRAPPED.
Module Export mathcomp_DOT_classical_DOT_functions.
Module Export mathcomp.
Module Export classical.
Module functions.
Include mathcomp_DOT_classical_DOT_functions_WRAPPED.functions.
End functions.
...
End mathcomp_DOT_classical_DOT_functions.
