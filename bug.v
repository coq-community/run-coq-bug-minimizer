...
(* coqc version 8.20+alpha compiled with OCaml 4.14.1
   ...
   Expected coqc runtime on this file: 35.753 sec *)
...
Definition inj {aT rT : nonPropType} {A : set aT} {f : {inj A >-> rT}} :
   {in A &, injective f}. exact (pcan_in_inj funoK). Defined.
...
Definition inj_hint {aT rT} {A : set aT} {f : {inj A >-> rT}} :
   {in A &, injective f}. exact (inj). Defined.
...
End SubType.
Definition to_setT {T} (x : T) : [set: T]. exact (@SigSub _ _ _ x (mem_set I : x \in setT)). Defined.
...
Section subfun.
Context {T} {A B : set T}.
Definition subfun (AB : A `<=` B) (a : A) : B. exact (SigSub (mem_set (AB _ (set_valP a)))). Defined.
...
Section set_val.
Context {T} {A : set T}.
Definition set_val : A -> T. exact (eqincl (set_mem_set A) \o val). Defined.
...
Definition finset_val {T : choiceType} {X : {fset T}} (x : X) : [set` X]. exact (exist (fun x => x \in [set` X]) (val x) (mem_set (valP x))). Defined.
Definition val_finset {T : choiceType} {X : {fset T}} (x : [set` X]) : X. exact ([` set_mem (valP x)]%fset). Defined.
...
Definition sigL (f : U -> V) : A -> V. exact (f \o set_val). Defined.
...
Definition valL_ (f : A -> V) : U -> V. exact (((@oapp _ _)^~ v) f \o 'oinv_set_val). Defined.
...
Definition sigR (f : {fun [set: U] >-> A}) (u : U) : A. exact (SigSub (mem_set ('funS_f I) : f u \in A)). Defined.
...
Definition valR_fun (f : U -> A) : {fun [set: U] >-> A}. exact ([fun of valR f]). Defined.
...
Definition valLR : (A -> B) -> U -> V. exact (valL \o valR_fun). Defined.
Definition valLRfun : (A -> B) -> {fun A >-> B}. exact (valLfun \o valR_fun). Defined.
...
Import GRing.Theory.
Definition cst {T T' : Type} (x : T') : T -> T'. exact (fun=> x). Defined.
