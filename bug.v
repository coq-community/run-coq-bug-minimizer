...
(* coqc version 8.16+alpha compiled with OCaml 4.13.0
   ...
   Expected coqc runtime on this file: 39.321 sec *)
...
exact (scmm Hdb fetch dec exec ammio procInit memInit).
Defined.
...
Definition kword: Type.
exact (Kami.Lib.Word.word sz).
Defined.
Definition kunsigned(x: kword): Z.
exact (Z.of_N (wordToN x)).
Defined.
Definition ksigned: kword -> Z.
Admitted.
Definition kofZ: Z -> kword.
exact (ZToWord sz).
Defined.
...
Section MkWords.
  Context {width : Z}.
Instance wordW: word.word width.
exact (word width).
Defined.
End MkWords.
